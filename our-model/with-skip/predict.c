...
float get_eligible_pages(float Ctime, float Ptime, float Cdirty, float Pdirty)
{
    if (Ctime >= 1) {
        ...
        if (Pdirty >= Sm) {
            ...
            //return predict_dirty(rms) - (rms * Sn);
        }
...
int main(int argc, char *argv[])
{
    ...
    for (iter = 1; iter <= 29; iter++) {
        ...
        if (iter > 1) {
            ...
            if (per_iter_time > 1) {
                //w = -0.2289 * (rate*8)/900 + 1.5881 * (dirty/1536.0) - 1.2391 * (edp/1536.0) + 0.1326 * Sm/dirty + 0.6847;
                w = -0.1875 * (rate*8)/900 + 0.2927 * (edp/1536.0) + 0.0902 * Sm/dirty + 0.7394;

            } else {
                //w = -0.1695 * (rate*8)/900 + 1.7226 * (dirty/1536.0) + 2.5569 * (edp/1536.0) - 0.0134 * (Um/dirty) + 0.003 * Un/(dirty-Um) + 0.6531;
                w = -0.1615 * (rate*8)/900 + 4.7881 * (edp/1536.0) - 0.0144 * (Um/dirty) + 0.003 * Un/(dirty-Um) + 0.6501;
                //w = -0.0629 * (rate*8)/900 + 2.8626 * (edp/1536.0) + 0.6803;
            }
        } else {
