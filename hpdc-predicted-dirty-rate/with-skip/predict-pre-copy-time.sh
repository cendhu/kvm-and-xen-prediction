...
do
    ...
    do
        ...
        do
            ...
            #wdr=`awk -f weighted-dirty-rate.awk ${wl[$windex]}/${rate[$rindex]}/$i/$mig_log_name`
            wdr=`awk -f weighted-dirty-rate.awk ${wl[$windex]}/${rate[$rindex]}/$i/$mig_log_name`
            s=`echo "scale=3; $1*($wdr/256.0) + 0.1*${dr[$windex]}" | bc`
            ...
            echo ${wname[$windex]}" "$s" "${ratem[$rindex]}" "$an >> hpdc-$1.txt
            ###For (M/R)###
            ...
            echo ${wname[$windex]}" "$s" "${ratem[$rindex]}" 1.PTpre "$pt" 2.ATpre "$tm" 3.ETpre "$er" 4.PETpre "$per" 5.More "$tmore"6.s>r:t 0 7.PTdown 0 8.ATdown "$atdown" 9.ETdown "$atdown" 10.PETdown 100 11.More 0 12.s>r:d 0 13.PVol "$pvol" 14.AVol "$avol" 15.EVol "$ver" 16.PEVol "$vper" 17.More "$vmore >> base-$1.txt
...
#<<CC
            ...
            #printing the measured pre-copy time, predicted, err and percentage of error
            echo ${wname[$windex]}" "$s" "${ratem[$rindex]}" 1.PTpre "$pt" 2.ATpre "$tm" 3.ETpre "$er" 4.PETpre "$per" 5.More "$tmore" 6.s>r:t "$esgrt" 7.PTdown 0 8.ATdown "$atdown" 9.ETdown "$atdown" 10.PETdown 100 11.More 0 12.s>r:d 0 13.PVol "$pvol" 14.AVol "$avol" 15.EVol "$ver" 16.PEVol "$vper" 17.More "$vmore >> effective-rate-$1.txt

        done
    done
done
./get_mt.sh base-$1.txt
./get_mt.sh hpdc-$1.txt
./get_mt.sh infocom-$1.txt
./get_mt.sh effective-rate-$1.txt
<<CC
