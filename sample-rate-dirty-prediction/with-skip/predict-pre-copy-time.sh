...
do
    ...
    do
        ...
        do
            ...
            echo "${wname[$windex]} ./hpdc-skip ${sz[$windex]} ${ratem[$rindex]} $it $tm $s 0 $dtd" >> execute-command.sh
            an=`./hpdc-skip ${sz[$windex]} ${ratem[$rindex]} $it $tm $s 0 $dtd`
            echo ${wname[$windex]}" "$s" "${ratem[$rindex]}" "$an >> hpdc.txt
            ###For (M/R)###
            ...
            pt=`echo "scale=3; 1024/${r[$rindex]}" | bc`
            ...
            echo ${wname[$windex]}" "$s" "${ratem[$rindex]}" 1.PTpre "$pt" 2.ATpre "$tm" 3.ETpre "$er" 4.PETpre "$per" 5.More "$tmore"6.s>r:t 0 7.PTdown 0 8.ATdown "$atdown" 9.ETdown "$atdown" 10.PETdown 100 11.More 0 12.s>r:d 0 13.PVol "$pvol" 14.AVol "$avol" 15.EVol "$ver" 16.PEVol "$vper" 17.More "$vmore >> base.txt
            ...
            vper=`echo "scale=3; ($ver/$avol)*100" | bc`
            ptdown=`echo "scale=10; (${sz[$windex]}/${r[$rindex]})*($sr)^($it+1)" | bc`
            ...
            #printing the measured pre-copy time, predicted, err and percentage of error
            echo ${wname[$windex]}" "$s" "${ratem[$rindex]}" 1.PTpre "$pt" 2.ATpre "$tm" 3.ETpre "$er" 4.PETpre "$per" 5.More "$tmore" 6.s>r:t " $isgrt " 7.PTdown "$ptdown" 8.ATdown "$atdown" 9.ETdown "$der" 10.PETdown "$dper" 11.More "$dmore" 12.s>r:d "$isgrd" 13.PVol "$pvol" 14.AVol "$avol" 15.EVol "$ver" 16.PEVol "$vper" 17.More "$vmore >> infocom.txt

#<<CC
            ###For  M/(R-S)###
            #get the value of R-S
            sr=`echo "(${ratem[$rindex]}/8)-($s) > 1" | bc`
            ...
            #printing the measured pre-copy time, predicted, err and percentage of error
            echo ${wname[$windex]}" "$s" "${ratem[$rindex]}" 1.PTpre "$pt" 2.ATpre "$tm" 3.ETpre "$er" 4.PETpre "$per" 5.More "$tmore" 6.s>r:t "$esgrt" 7.PTdown 0 8.ATdown "$atdown" 9.ETdown "$atdown" 10.PETdown 100 11.More 0 12.s>r:d 0 13.PVol "$pvol" 14.AVol "$avol" 15.EVol "$ver" 16.PEVol "$vper" 17.More "$vmore >> effective-rate.txt
        done
    done
done
./get_mt.sh base.txt
./get_mt.sh hpdc.txt
./get_mt.sh infocom.txt
./get_mt.sh effective-rate.txt

./process.sh base.txt
./process.sh hpdc.txt
./process.sh infocom.txt
./process.sh effective-rate.txt
<<CC
